...
{
    public class Classic : IMinimaxExecutor<int>
    {
        ...
        public Classic(Roll maximizer, Roll minimizer, int maxDepth, GlobalConfig config)
        {
            Maximizer = maximizer;
            Minimizer = minimizer;
            MaxDepth = maxDepth;
        ...
        private int UtilityFunction(IGameState gameState, int depth)
        {
        ...
        IMinimaxResponse<int> minimax(IGameState gameState, int depth)
        {
            ...
            foreach (Match match in matches)
            {
                ...
                IMinimaxResponse<int> response = minimax(newGameState, depth + 1);

                if (maximizerTurn)
                {
                    if (response.Utility > better.Utility)
                    {
